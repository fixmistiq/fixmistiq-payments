...
def debug_redis():
    ...
    results = {}
    try:
        # Test 1: simple key no colons
        r1 = http_requests.get(
            f"{UPSTASH_URL}/set/testkey/testvalue",
            headers={"Authorization": f"Bearer {UPSTASH_TOKEN}"},
            timeout=10
        )
        results["set_simple"] = {"status": r1.status_code, "body": r1.text}

        # Test 2: read it back
        r2 = http_requests.get(
            f"{UPSTASH_URL}/get/testkey",
            headers={"Authorization": f"Bearer {UPSTASH_TOKEN}"},
            timeout=10
        )
        results["get_simple"] = {"status": r2.status_code, "body": r2.text}

        # Test 3: with colon (via our helper)
        redis_set("debug:user1", "hello_value")
        results["set_via_helper"] = "called"
        results["get_via_helper"] = redis_get("debug:user1")

        # Config check
        results["url_set"] = bool(UPSTASH_URL)
        results["url_value"] = UPSTASH_URL[:40] if UPSTASH_URL else "MISSING"
        results["token_set"] = bool(UPSTASH_TOKEN)
        results["token_length"] = len(UPSTASH_TOKEN) if UPSTASH_TOKEN else 0
    except Exception as e:
        results["error"] = str(e)
    return results
...
def test_activate(user_id: str, days: int = 1, plan: str = "monthly"):
    if plan == "yearly":
        days = 365
    expiry = datetime.now() + timedelta(days=days)
    redis_set(f"premium:{user_id}", expiry.isoformat())
    return {"user_id": user_id, "premium": True, "expires_on": expiry.isoformat()}
